...
* 2013.4.23
*/
void * mms_connect_thread(void *arg)
{

    struct mms_connect_arg cdata;
    struct mms_connect_arg *parg = (struct mms_connect_arg *)arg;
    memset(&cdata, 0, sizeof(struct mms_connect_arg));
    cdata.io = parg->io;
    cdata.data = parg->data;
    cdata.url = parg->url;
    cdata.bandwidth = parg->bandwidth;

    printf("mms_connect_process\n");
    return (void *)mms_connect_process(cdata.io, cdata.data, cdata.url, cdata.bandwidth);
...
mms_t *mms_connect(mms_io_t *io, void *data, const char *url, int bandwidth)
{
    ...
    cdata.data = data;
    cdata.url = url;
    cdata.bandwidth = bandwidth;

    err = pthread_create(&tid, NULL, mms_connect_thread, &cdata);
    if (err != 0)
    {
...
mms_t *mms_connect_process (mms_io_t *io, void *data, const char *url, int bandwidth)
{
