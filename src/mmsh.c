...
static int mmsh_connect_int_process (mms_io_t *io, mmsh_t *this, off_t seek, uint32_t time_seek) {
  int    i;
  int    video_stream = -1;
  int    audio_stream = -1;
...
/*
* thread process connect
* bug report: xubin@linuxdeepin.com
* 2013.4.23
*/
void * mmsh_connect_thread(void *arg)
{

    struct mmsh_connect_arg cdata;
    struct mmsh_connect_arg *parg = (struct mmsh_connect_arg *)arg;
    memset(&cdata, 0, sizeof(struct mmsh_connect_arg));
    cdata.io = parg->io;
    cdata.this = parg->this;
    cdata.seek = parg->seek;
    cdata.time_seek = parg->time_seek;

    printf("mmsh_connect_process\n");
    return (void *)mmsh_connect_int_process(cdata.io, cdata.this, cdata.seek, cdata.time_seek);

}
/*
* create a thread to process connect
* bug report: xubin@linuxdeepin.com
* 2013.4.23
*/
static int mmsh_connect_int(mms_io_t *io, mmsh_t *this, off_t seek, uint32_t time_seek)
{
    ...
    cdata.this = this;
    cdata.seek = seek;
    cdata.time_seek = time_seek;

    err = pthread_create(&tid, NULL, mmsh_connect_thread, &cdata);
    if (err != 0)
    {
